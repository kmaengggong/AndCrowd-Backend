...
class DynamicCrowdSponsorControllerTest {
    ...
    void deleteCrowdSponsorTest() throws Exception {
        ...
        // then
        List<DynamicCrowdSponsorDTO.FindById> afterSponsorList = dynamicCrowdSponsorRepository.findAll(crowdId);
        assertEquals(2, afterSponsorList.size());
        assertThat(dynamicCrowdSponsorRepository.findBySponsorId(crowdId, sponsorId).isDeleted()).isTrue();
    }
}